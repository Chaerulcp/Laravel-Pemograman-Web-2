                ...
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.reservasi.update', $reservasi->id) }}">
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="email" id="email" name="email" value="{{ $reservasi->email }}"
                                    class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                            </div>
                            @error('email')
                                <div class="text-sm text-red-400">{{ $message }}</div>
                        ...
                        <div class="sm:col-span-6">
                            <label for="no_hp" class="block text-sm font-medium text-gray-700"> No Telp
                            </label>
                            <div class="mt-1">
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                ...
                                value="{{ \Carbon\Carbon::parse($reservasi->tanggal_reservasi)->format('Y-m-d\TH:i') }}"
                                    class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                            </div>
                            ...
                            <div class="mt-1">
                                <select id="table_id" name="table_id" class="form-multiselect block w-full mt-1">
                                    @foreach ($table as $tables)
                                        <option value="{{ $tables->id }}" @selected($tables->id == $reservasi->tables_id)>
                                            {{ $tables->nama }}
                                            ({{ $tables->kapasitas }} Orang)
                                        </option>
                                    @endforeach
                                </select>
                            </div>
