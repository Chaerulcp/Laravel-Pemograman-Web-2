                ...
                <div class="flex flex-col md:flex-row">
                    ...
                    <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
                        <div class="w-full">
                            ...
                            <form method="POST" action="{{ route('reservasi.store.step.two') }}">
                                @csrf
                                        ...
                                        <select id="table_id" name="table_id" class="form-multiselect block w-full mt-1">
                                            @isset($tables)
                                                @foreach ($tables as $table)
                                                    <option value="{{ $table->id }}" @if($table->id == $reservasi->table_id) selected @endif>
                                                        {{ $table->nama }} ({{ $table->kapasitas }} Orang)
                                                    </option>
                                                @endforeach
                                            @endisset
                                        </select>
                                ...
                                <div class="mt-6 p-4 flex justify-between">
                                    <a href="{{ route('reservasi.step.one') }}"
